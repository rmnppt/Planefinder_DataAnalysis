...
Lat <- c(50, 58) 
Lon <- c(-10, 2)
flights <- flights %>% filter(Longitude > Lon[1] & 
                                Longitude < Lon[2] &
                                Latitude > Lat[1] &
                                Latitude < Lat[2])
...
theseMinsThin <- theseMins[which(theseMins %% window == 0)]
...
for(i in theseMins){
  subs[[i]] <- flights %>% 
    ...
    mutate(ID = paste0(Registration, Callsign, ADSHEX))
  cat("_", i, "_")
}
...
interval = 0.1, movie.name = "planeMovie_UK.gif", 
